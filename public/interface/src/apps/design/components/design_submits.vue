...
let comp_taskStatus = {
  template:
    '<el-tag :type="getTypeByStatus(status)">{{getTextByStatus(status)}}</el-tag>',
  props: ["status"],
  data: function() {
    return {
      defaultText: "",
...
export default {
  ...
  methods: {
    ...
    getTableDataHandler(data = {}, func) {
      ...
      //                console.log(data)
      vm.$http
        .post(vm.serverUrl + "get_list", data)
        .then(res => {
    ...
    finishIt(id) {
      let vm = this;
      vm.$http
        .post(vm.serverUrl + "finished", { id: id.toString() })
        .then(response => {
          if (response.result.code === 1) {
    ...
    batchDeleted() {
      ...
      if (vm.tableSelected.length > 0) {
        ...
        vm
          .$confirm("是否删除?", "删除[" + ids + "]号订单")
          .then(_ => {
            vm.$http
              .post(vm.serverUrl + "deleted", { id: ids.toString() })
              .then(response => {
                if (response.result.code === 1) {
